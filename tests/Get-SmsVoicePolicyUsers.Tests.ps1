Describe 'SMS/Voice policy scanner' {
    BeforeEach {
        ...
        function Invoke-ScannerFixture($AuthenticationPolicy, $SmsPolicy, $VoicePolicy, $RegistrationDetails = @()) {
            ...
            $rows = @(Get-ChildItem -Path $fixtureDirectory -Filter '*.csv' | ForEach-Object { Import-Csv -LiteralPath $_.FullName })
    ...
    It 'exports only users with an explicit SMS or Voice preferred method' {
        $smsPolicy = [PSCustomObject]@{
            State = 'enabled'
            IncludeTargets = @([PSCustomObject]@{ TargetType = 'group'; Id = 'all_users' })
        }
        $registrationDetails = @(
            [PSCustomObject]@{
                Id = 'sms-user'; UserDisplayName = 'SMS User'; UserPrincipalName = 'sms@example.test'; UserType = 'member'
                MethodsRegistered = @('mobilePhone'); UserPreferredMethodForSecondaryAuthentication = 'sms'
                SystemPreferredAuthenticationMethods = @('sms'); LastUpdatedDateTime = '2026-09-14T12:00:00Z'
            },
            [PSCustomObject]@{
                Id = 'phone-only-user'; UserDisplayName = 'Phone Only'; UserPrincipalName = 'phone@example.test'; UserType = 'member'
                MethodsRegistered = @('mobilePhone'); UserPreferredMethodForSecondaryAuthentication = 'push'
                SystemPreferredAuthenticationMethods = @('push'); LastUpdatedDateTime = '2026-09-14T12:00:00Z'
            }
        )
        $result = Invoke-ScannerFixture $authPolicy $smsPolicy $disabledPolicy $registrationDetails
        $userRows = @($result.Rows | Where-Object { $_.UserPrincipalName })
        $userRows.Count | Should Be 1
        $userRows[0].UserPrincipalName | Should Be 'sms@example.test'
        $userRows[0].MatchingSmsVoiceMethods | Should Be 'sms'
        $result.Text | Should Match 'SMS/Voice user report: 1 users'
    }
