...
- Exports a user list for people whose preferred or system-preferred MFA method is SMS or Voice, including their registered methods for context
...
- This is a read-only policy scanner with a targeted user registration report. It does not expand policy-group membership, calculate effective user counts after exclusions, or read sign-in activity. An `AllUsers` policy-target CSV row is not an enumerated list of users.
- The SMS/Voice user CSV lists users whose preferred or system-preferred MFA method is explicitly `sms`, `voiceMobile`, `voiceAlternateMobile`, or `voiceOffice`. It is not evidence of recent sign-in usage. A mobile number appearing in `RegisteredMethods` alone is not enough to establish SMS or Voice use, so it does not by itself include a user in this report.
...
| `AuditLog.Read.All` | Read per-user authentication registration details |

**Minimum Entra role:** Global Reader, Reports Reader, Security Reader, or Security Administrator. The policy portion can be read with Authentication Policy Administrator, but the user registration report also requires a role supported by the Graph reporting API.
