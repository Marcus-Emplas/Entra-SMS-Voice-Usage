<#
.SYNOPSIS
    Shows SMS/Voice auth method policy scope, SMS/Voice user registrations, and retirement guidance for your Entra tenant.
.PARAMETER TenantId
...
# Connect
$connectParams = @{ Scopes = @("Policy.Read.All", "Group.Read.All", "AuditLog.Read.All") }
...
try {
    ...
    $smsVoiceUsers = foreach ($user in $registrationDetails) {
        $preferredMethod = Get-PolicyProperty $user 'userPreferredMethodForSecondaryAuthentication'
        $systemPreferredMethods = @(Get-PolicyProperty $user 'systemPreferredAuthenticationMethods')
        $matchingMethods = @($systemPreferredMethods | Where-Object { $_ -in $smsVoiceMethods })

        if ($preferredMethod -in $smsVoiceMethods -or $matchingMethods.Count -gt 0) {
            $registeredMethods = @(Get-PolicyProperty $user 'methodsRegistered')
            [PSCustomObject]@{
                UserId = Get-PolicyProperty $user 'id'
                DisplayName = Get-PolicyProperty $user 'userDisplayName'
                UserPrincipalName = Get-PolicyProperty $user 'userPrincipalName'
                UserType = Get-PolicyProperty $user 'userType'
                RegisteredMethods = $registeredMethods -join '; '
                PreferredMfaMethod = $preferredMethod
                SystemPreferredMfaMethods = $systemPreferredMethods -join '; '
                MatchingSmsVoiceMethods = (@($preferredMethod) + $matchingMethods | Where-Object { $_ -in $smsVoiceMethods } | Select-Object -Unique) -join '; '
                LastUpdatedDateTime = Get-PolicyProperty $user 'lastUpdatedDateTime'
            }
        }
    }
    ...
    Write-Host "Exported to: $userExportPath" -ForegroundColor Green
} catch {
